...
void Introduction::Init()
{
    ...
    m_introTitle.setPosition(m_context->m_window->getSize().x / 2 - 80.f,
                             m_context->m_window->getSize().y / 2 - 530.f);
    m_introTitle.setCharacterSize(84);
    ...
    sf::String contentText = L"某天，你突然發現你深陷在夢中，醒不過來。現實中，你只是一個混到畢業的資管系學生，"
                             "目前是個什麼";
    contentText += '\n';
    contentText += L"程式都不會打的無業遊民。你以為在夢裡就能有錢有房有車有女友，結果卻發現在夢中也不能為所欲為，";
    contentText += '\n';
    contentText += L"每天只能去超商買即期食品...今天一如往常騎著腳踏車，到超商買微波食品的路上，不只沒有幸運的事情";
    contentText += '\n';
    contentText += L" 發生，你還為了閃躲走在自行車道的移動式路障，整個人摔進了花叢中，雖然沒有受傷，但腳踏車的車籃";
    contentText += "\n";
    contentText += L"直接撞成一個茶壺的形狀當你心疼的摸著車籃的時候，你發現，原來是地上那個破茶壺，把你的車籃弄成";
    contentText += "\n";
    contentText += L"這樣，一氣之下，用力地踹了一腳，你「噢嗚！」了一聲，卻也聽到破茶壺「噢嗚！」了一聲，轉頭想拔";
    contentText += "\n";
    contentText += L"腿就跑的你，發現忘了騎腳踏車心理雖然很害怕，但是但是更害怕自己什麼都沒有了，所以不能連腳踏車";
    contentText += "\n";
    contentText += L"都沒有。正當你一回頭，就被吸進了茶壺的世界，這裡黑漆漆的，只有一台遊戲機，上面寫著：想想你的";
    contentText += "\n";
    contentText += L"大學生活到底經歷了什麼吧！如果能夠破完全部的關卡，我就考慮實現你一個願望～～";
    //contentText += "\n\n";
    ...
    m_introContent.setPosition(m_context->m_window->getSize().x / 2 - 200.f,
                                   m_context->m_window->getSize().y / 2 -100.f);
    m_introContent.setCharacterSize(40);
    sf::String ruleContext;
    ruleContext += L"規則";
    ruleContext += '\n';
    ruleContext += L"1. 每次遊戲要答對由簡至難共 7 道題目，每答對一題往上一等級，反之往後退一級。";
    ruleContext += '\n';
    ruleContext += L"2. 每次遊戲不能錯超過 5 題，超過的話被停權 30 秒。";
    ruleContext += '\n';
    ruleContext += L"3. 勝利條件：答對等級 7 的題目，便可以像神燈許一次願望。";
    ruleContext += '\n';
    m_introContentRule.setFont(m_context->m_assets->GetFont(MAIN_FONT));
    m_introContentRule.setString(ruleContext);
    m_introContentRule.setFillColor(sf::Color(121,2,2));
    m_introContentRule.setOrigin(m_introContentRule.getLocalBounds().width / 2,
                             m_introContentRule.getLocalBounds().height / 2);
    m_introContentRule.setPosition(m_context->m_window->getSize().x / 2 - 350.f,
                                   m_context->m_window->getSize().y / 2 + 330.f);
    m_introContentRule.setCharacterSize(40);
...
void Introduction::Draw()
{
    ...
    m_context->m_window->draw(m_introContentRule);
